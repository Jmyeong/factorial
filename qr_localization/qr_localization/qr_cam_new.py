...
class GetRange:
    ...
    def qr_detection(self):
        ...
        retval, points, decoded = qr_detector.detectAndDecode(self.undistorted)

        if decoded is None: decoded = 0
    ...
    def get_angle(self):
        ...
        if self.points is not None and len(self.points) >= 4:
            ...
            center_y = np.mean(self.points[:, 0, 1])
...
class QRCamera(Node):
    def __init__(self):
        ...
        self.qr_pub = self.create_publisher(LaserScan, '/coord_calc', qos)
    ...
    def lidar_callback(self, lidar_msg):
        ...
        if self.frame is not None:
            ...
            if self.qr.points is not None:
                self.qr.get_angle()
                qr_infos = [self.qr.center_x, self.center_y self.qr.decoded, self.qr.qr_angle]
                qr_coord_msg = LaserScan
                qr_coord_msg.header = lidar_msg.header
                qr_coord_msg.angle_min = new_minang
                qr_coord_msg.angle_max = new_maxang
                qr_coord_msg.angle_increment = lidar_msg.angle_increment
                qr_coord_msg.time_increment = lidar_msg.time_increment
                qr_coord_msg.scan_time = lidar_msg.scan_time
                qr_coord_msg.range_min = lidar_msg.range_min
                qr_coord_msg.range_max = lidar_msg.range_max
                qr_coord_msg.ranges = qr_infos
                qr_coord_msg.intensities = []

                self.qr_pub.publish(qr_coord_msg)
